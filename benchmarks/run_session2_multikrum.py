...
CHECKPOINT_EVERY   = 10
CHECKPOINT_DIR     = Path("checkpoints/session2_multikrum")
...
def main():
    ...
    log_path = CHECKPOINT_DIR / "session2_multikrum.log"
    tee = TeeLogger(str(log_path))
    sys.stdout = tee
    
    print("=" * 70)
    print("SESSION 2: MULTI-KRUM + BLIND DIFFUSION")
    ...
    for rnd in range(1, NUM_ROUNDS + 1):
        t_rnd = time.time()
        print(f"\n--- Round {rnd:3d}/{NUM_ROUNDS} [Multi-Krum + Diffusion] ---")
        ...
        for cid in range(TOTAL_CLIENTS):
            ...
            # NO blockchain gating -> Blind Diffusion
            client = FLClient(
                client_id=cid + 1,
                model=global_model,
                ...
                blockchain_manager=None, # Causes Blind LDM
        ...
        all_sizes   = [r[2] for r in results]
        
        # Multi-Krum Aggregation (f=2, k=7)
        global_model = multi_krum_aggregate(global_model, all_weights, all_sizes, f=NUM_MALICIOUS, k=7)
